		...
		{	
			...
			if(proc == NULL)
			{
				...
				if(bAutoDelete)
				{
					#ifdef _DEBUG
					TCHAR szMessage[256];
					_stprintf_s(szMessage, _T("Deleting Run Object of thread , handle = %d, id = %d\n"), 
					 hThread, dwThreadId);
					wprintf(szMessage);
					#endif
				...
				else
				{
					#ifdef _DEBUG
					TCHAR szMessage[256];
					_stprintf_s(szMessage, _T("Not Deleted Run Object of thread , handle = %d, id = %d\n"), 
					 hThread, dwThreadId);
					wprintf(szMessage);
					#endif
				}
...
bool CThreadPool::Create()
{
	...
	_stprintf_s(m_szPoolName, _T("Pool%d"), m_lInstance);
	...
	{
		_stprintf_s(szEvtName, _T("PID:%d IID:%d TDX:%d"), GetCurrentProcessId(), CThreadPool::m_lInstance, nIndex /* thread index*/);
		...
		if(hThread)
		{
			...
			#ifdef _DEBUG
			TCHAR szMessage[256];
			_stprintf_s(szMessage, _T("Thread created, handle = %d, id = %d\n"), 
					  hThread, dwThreadId);
			wprintf(szMessage);
			#endif
		}
...
void CThreadPool::FinishNotify(DWORD dwThreadId)
{
	...
	{			
		...
		wprintf(_T("No matching thread found."));
		return;
	}
	else
	{	
		...
		#ifdef _DEBUG
		TCHAR szMessage[256];
		_stprintf_s(szMessage, _T("Thread free, thread id = %d\n"), dwThreadId);
		wprintf(szMessage);
		#endif
...
void CThreadPool::BusyNotify(DWORD dwThreadId)
{
	...
	{		
		...
		#ifdef _DEBUG
		TCHAR szMessage[256];
		_stprintf_s(szMessage, _T("Thread busy, thread id = %d\n"), dwThreadId);
		wprintf(szMessage);
		#endif
